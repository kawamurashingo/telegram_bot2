...
## telegram bot id
# BOT_ID="XXXX:XXXX" # Telegram botのIDを設定。未使用の場合はコメントアウト。

DIR=`dirname $0` # 現在のスクリプトのディレクトリを取得。

cd ${DIR} # スクリプトのディレクトリに移動。
...
FILE_DATE=`date +%Y%m%d%H%M` # 現在の日付と時間を取得。
OLD_FILE_DATE=`ls -ltr ./FILE/ | tail -n1 | awk '{print $9}'` # 最新のファイル日付を取得。

# Googleカレンダーから翌日のスケジュールを取得し、整形。
